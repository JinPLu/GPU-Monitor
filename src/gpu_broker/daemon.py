...
DAEMON_LABEL = "local.gpu-broker.daemon"
...
def _loopback_url(value: str) -> tuple[str, int, str]:
    ...
    if parsed.scheme != "http" or parsed.hostname not in {"127.0.0.1", "::1", "localhost"}:
        raise DaemonError("macOS daemon auto-start requires a loopback http GPU_BROKER_URL")
    if parsed.username or parsed.password or parsed.query or parsed.fragment:
        raise DaemonError("GPU_BROKER_URL must not include credentials, query, or fragment")
    if parsed.path not in {"", "/"}:
        raise DaemonError("GPU_BROKER_URL must not include an API path")
...
def _daemon_executable(environment: Mapping[str, str], home: Path) -> Path:
    configured = environment.get("GPU_BROKER_DAEMON_EXECUTABLE")
    candidates = [
        Path(configured).expanduser() if configured else None,
        home / ".local/share/uv/tools/gpu-broker/bin/gpu-broker",
        Path(shutil.which("gpu-broker") or ""),
        Path(sys.executable).with_name("gpu-broker"),
    ]
    for candidate in candidates:
        if candidate and candidate.is_file() and os.access(candidate, os.X_OK):
            return candidate.resolve()
    raise DaemonError(
        "cannot find a durable gpu-broker executable; install it with "
        "`uv tool install --force /path/to/gpu-broker`"
    )
...
def resolve_daemon_config(
    ...
    return DaemonConfig(
        ...
        log_dir=home / "Library/Logs/GPU Broker",
    ...
    def _prepare_data(self, source_root: Path | None) -> dict[str, bool]:
        ...
        if not self.config.inventory_path.is_file():
            if source_root is None:
                raise DaemonError(
                    f"missing {self.config.inventory_path}; run `gpu-broker daemon install "
                    "--source-root /path/to/gpu-broker` once"
                )
            source_inventory = source_root.expanduser().resolve() / "configs/inventory.yaml"
        ...
        if not self.config.database_path.exists() and source_root is not None:
            source_database = (
                source_root.expanduser().resolve() / "state/gpu-broker.sqlite3"
            )
...
def ensure_broker_ready_for_mcp() -> None:
    if sys.platform != "darwin":
        return
    if os.environ.get("GPU_BROKER_AUTOSTART", "1").lower() in {"0", "false", "no"}:
        return
