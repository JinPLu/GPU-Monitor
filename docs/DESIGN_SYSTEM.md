# ServerPilot 桌面设计系统

本文是 macOS 与 Windows App 的共同界面契约。目标不是展示最多字段，而是让用户在较低认知负担下完成三件事：**看清资源、申请资源、追溯归属**。

## 1. 产品边界

- 单用户、本机控制面、多服务器、多 Agent。
- App 是观察与操作界面，不是聊天工具，也不是任务执行器。
- 项目与任务是公开 GUI 中唯一的工作身份。GUI 不展示 Agent、actor、owner、收件箱或消息入口。
- 资源真相来自同一 REST 状态；界面不得自行用“容量减用量”推算可分配资源。

## 2. 视觉原则

### 仅浅色

- App 固定使用浅色外观，不跟随系统深色模式。
- 主窗口与主要容器使用不透明表面。禁止全窗毛玻璃、实时模糊、背景图片、渐变光晕和大阴影。
- 页面背景为低亮度中性灰，内容表面略亮；层级依靠间距、字重和细分隔线，而不是堆叠卡片。
- macOS 使用原生控件与 SF Symbols；Windows 使用系统 WebView2 窗口和与 macOS 一致的浅色信息密度、标签、状态色和操作顺序，不创建第二套产品语言。

### Beszel 式信息密度

- 资源总览以紧凑、可排序的表格为主，不改成大卡片阵列。
- 正常状态让数字和短标签承担信息；解释性句子只用于错误、空状态和不可逆确认。
- 一行显示一个服务器。列宽由整张表统一决定，不允许单行变形或局部切换布局。
- 主要值完整优先，次要说明才可截断。悬停帮助与辅助功能名称保留完整语义。

## 3. 设计令牌

### 颜色

| 用途 | 颜色 | 规则 |
| --- | --- | --- |
| 正常 / 低压力 | `#339653` | 状态点与低压力进度柱 |
| 注意 / 中压力 | `#AA7C00` | 状态点与中压力进度柱 |
| 错误 / 高压力 | `#FB2C37` | 状态点、错误与高压力进度柱 |
| 交互 | macOS `controlAccentColor` | 选中、主按钮、焦点和链接 |
| 主文字 | `labelColor` | 标题、SSH、项目、任务、型号和数字 |
| 次文字 | `secondaryLabelColor` | 单位、辅助状态、时间和空值说明 |
| 轨道 | 中性浅灰 | 所有资源进度柱的未使用部分 |

数字保持中性色。压力颜色只放在状态点和进度柱上。颜色必须同时配合文字、图标或数值，不能独自传达状态。

三个状态色是图形对象，必须对内容表面（`#F6F7F9`）和页面底色（`#E4E7EC`）都不低于 WCAG 1.4.11 的 3:1。当前取值实测为 3.48 / 3.50 / 3.55 与 3.01 / 3.03 / 3.07，三者视觉权重一致。调整色值时只改明度、保持色相，并重新测量这两个背景；不得靠加大面积规避阈值。它们按上一条只用于状态点和进度柱，因此按图形阈值取值；一旦被用到文字上就会低于正文所需的 4.5:1，这也是上一条禁止该用法的原因之一。

### 字体

- 页面标题：系统字体 20–22 pt，semibold。
- 表头：系统字体 12 pt，semibold，必须明显高于辅助文字。
- 行主值：系统字体 12 pt，medium 或 semibold。
- 行辅助值：系统字体 9.5–10.5 pt，regular。
- SSH、GPU ID、时间和数字：系统等宽字体或 tabular figures。
- 禁止为了“丰富”而给同一概念换词。

### 尺寸

- 8 pt 基础网格。
- 页面边距 16–20 pt；区块间距 12–16 pt；行内间距 6–10 pt。
- 表格行高紧凑，但整行点击区域不得小于 36 pt。
- 指标进度柱约 40 pt 宽，和数字并排。
- 容器圆角 10–12 pt，边框为 1 px 中性细线。无重阴影。

## 4. 信息架构

侧栏只保留三个一级页面：

1. **服务器**
2. **使用情况**
3. **设置**

窗口标题、侧栏选中项和页面标题不得重复表达同一信息。工具栏只保留当前页的必要动作。

## 5. 服务器页

### 顶部

- 页面标题：`服务器`
- 更新时间：`更新于 HH:mm`
- 主要动作：`添加服务器`、`申请 GPU`、`刷新`
- 搜索占位：`搜索 SSH、GPU、项目或任务`
- 筛选：`全部 / 有空闲 GPU / 任务占用 / 占卡 / 连接失败`
- 排序菜单作为窄窗口补充入口，不能取代表头排序。

`刷新`表示重新读取控制面的当前状态。服务器观测由设置中的 5 / 10 / 30 秒采集间隔驱动；刷新不伪造一次新观测。

### 表格列

列顺序固定：

| 列 | 可见内容 | 表头图标 | 默认排序 |
| --- | --- | --- | --- |
| 服务器 | 状态点 + 完整 SSH 指令 | `server.rack` | 升序 |
| 项目 / 当前任务 | 项目名；下一行当前任务 | `folder` | 升序 |
| GPU 配置 | 型号；下一行卡数与单卡显存 | `square.stack.3d.up` | 升序 |
| 空闲 | 空闲卡数 / 总卡数 | `checkmark.circle` | 降序 |
| GPU 利用率 | 利用率 + 加宽进度柱 | 无（以完整标题说明） | 降序 |
| 显存占用率 | 占用率 + 加宽进度柱 | 无（以完整标题说明） | 降序 |
| CPU 负载 | 负载 + 加宽进度柱 | 无（以完整标题说明） | 降序 |
| 内存占用率 | 占用率 + 加宽进度柱 | 无（以完整标题说明） | 降序 |

带图标的表头统一使用 SF Symbols 线框或层级渲染，标题与图标之间保留 6 pt。四项资源指标表头在标准宽度下不带图标；只有切换到紧凑布局、完整标题被压缩时才补图标，用于替代被截断的标题语义。资源指标以完整标题优先，避免以缩写或单个名词造成歧义。未选中列不常驻上下箭头；当前排序列使用浅色圆角底和单个 `↑` 或 `↓`。再次点击同一表头切换方向。

标准宽度下，整张表必须填满可用窗口：服务器、GPU 配置、空闲和四项资源指标使用稳定列宽，项目 / 当前任务列吸收全部剩余宽度；GPU 配置保持在项目列之后的较靠左位置。表格内宽低于这些最小列宽时才切换紧凑布局，不能留下非必要的右侧空白。长项目或任务只在本列尾部截断。资源列直接使用完整的专业名称：GPU 利用率、显存占用率、CPU 负载和内存占用率。

服务器页的四项资源指标和对应排序统一标示为**近 10 分钟均值**；GPU、主机 CPU 负载与主机内存都来自同一滚动观察窗口。没有该窗口数据时显示`—`，不得以瞬时值替代。服务器详情页不重复汇总这些指标；逐 GPU 显存环图以打开详情时的瞬时观测绘制。

### 状态用语

| 状态 | GUI 用语 |
| --- | --- |
| 可参与新分配 | `空闲` |
| 有项目任务 | `任务占用` |
| 已申领但尚未观察到任务 | `占卡`，仍属于已分配资源，不等于空闲卡 |
| 未托管进程 | `未归属占用` |
| 明确归属冲突 | `归属冲突` |
| 无新观测 | `无响应` 或 `无最新数据` |

禁止使用泛化的`待处理`、`快照过期`、`fail-closed`或`允许延迟`。正常状态不显示说明段落；异常只显示原因和可执行下一步。

## 6. 服务器详情

详情以独立桌面弹窗打开，按以下顺序排列：

1. 状态、更新时间与完整 SSH 指令
2. 紧凑操作栏：申请 GPU、开始 / 结束占卡，以及「编辑或移除服务器」
3. 远端工作区；仅在异常或需要人工回收时追加说明和操作，不重复显示独立的占卡摘要
4. 逐 GPU 显存状态：顶部以紧凑计数展示`空闲 / 占卡 / 繁忙 / 错误`，下面恢复逐 GPU 横向信息卡。每张卡左侧使用约 40 pt 的环形图，沿用旧版逐卡组件的圆角端点、内圈浅色填充和 GPU 编号；环的当前填充比例与绿 / 黄 / 红颜色沿用资源表显存占用率阈值。右侧只保留状态标签、当前显存量和占用百分比，不显示型号、UUID 或任务叙述。不得使用四张等权彩色状态面板，也不得把全部 GPU 合并成一张状态饼图
5. 历史图与 `1h / 6h / 24h`，固定为 CPU、内存、GPU 利用率和显存占用率的 2×2 排列

详情不能挤压服务器列表，也不能在宽窗口退化为右侧 inspector。弹窗优先给逐 GPU 状态图、历史图和操作留出宽度；关闭按钮与 Escape 都能退出。`开始占卡` / `结束占卡`只切换 ServerPilot 的逐卡空闲占卡策略：不得暗示会停止已有任务；结束前必须说明该事实并确认。若工作负载租约卡在 `CONFLICT`，显示`清理遗留归属`，先做一次新鲜采集，只有确认所有相关 GPU 都没有进程时才释放。

逐卡策略必须隔离故障：一张 GPU 处于冲突、忙碌或 stale 时，只跳过该卡，不能把同一服务器的其它空闲 GPU 一并标成不可启动。详情页对遗留租约提供人类可见的回收动作；它依据新鲜进程观测，不把 0% 利用率单独当作“没有任务”。

历史图包括 CPU、内存、GPU 利用率和显存占用率。多张 GPU 使用稳定的不同颜色，并共享同行图例。采样中断时折线必须断开；缺值不能补成 0。悬停只显示当前时间点的必要值。

## 7. 使用情况页

- 页面标题：`使用情况`
- 默认按项目组织，只展示当前项目、当前任务和分配的 GPU。
- 正常行用短标签和资源图标表达，不重复解释 lease 生命周期。
- 空状态只显示：`当前没有资源分配。`，并提供`申请 GPU`。
- 不提供 Agent 视图、消息、协调链接、聊天或历史任务流水。

## 8. 设置页

只显示：

- `本机服务地址`
- `数据采集间隔`：`5 秒 / 10 秒 / 30 秒`
- `版本`

采集间隔是持久化设置。写入成功后才更新界面值；失败时保留旧值并显示一条具体错误。测试夹具可以展示实际值，但必须只读，不能伪装写入成功。

## 9. 表单、错误与确认

- 添加服务器：以 SSH 指令为主要输入，不重复展示可从指令直接读出的 host、port、user。
- 申请 GPU：正常态只显示项目、任务、GPU 数量和可选服务器；说明文字仅跟随字段错误。
- 错误：一句说明发生了什么，一句说明下一步。不要同时堆叠 banner、卡片和状态标签。
- 空状态：一句事实 + 一个主动作。
- 删除服务器放在编辑页底部「危险操作」区；确认框以取消为默认，并说明只停止本机监控/协调、不停止远端进程。预约与维护公共页只读，不提供创建或取消表单。

## 10. 交互与可访问性

- 搜索、筛选、所有可排序表头、服务器行、返回按钮和主要动作均可键盘到达。
- 每个图标按钮提供可访问名称；指标提供文字值与单位。
- 焦点环使用原生样式，不用颜色变化替代焦点。
- 关闭装饰性动画。状态刷新使用无位移更新；Reduce Motion 下不得依赖动画解释变化。
- 最小验收视口为 1024×640，同时检查 1280×800 与 1440×820。

## 11. 数据与能力边界

- App 只消费 loopback REST/domain 状态，不直连 SQLite 或 SSH。
- stale、unknown、unmanaged、conflict 和 maintenance 资源不参与分配。
- lease 协调归属，不启动、停止或抢占项目任务。
- Adapter ID 只用于诊断 provenance，不成为 GUI 工作流或资源身份。
- 外部 SchedulerTarget 与普通服务器分开；`PENDING` 作业不表示已获得裸机 GPU。

任何新增页面、字段或说明，都必须回答一个问题：它是否直接帮助用户看清、申请或追溯资源。不能回答时，不加入 GUI。
