...
2. `gpu_apply(server_id?, gpu_count=1, task?)` 直接申请。`task` 应使用用户给定的任务名，或把当前工作目标概括成简短、可读的任务说明；不要尝试读取客户端 UI 标题。未提供时记录为“未命名任务”。ServerPilot 自动选择具体 GPU；Agent 不提供 GPU ID。单 endpoint 申请成功时返回顶层 `lease_id`、`server_id`、`workspace_path`、`cuda_visible_devices` 与 `gpus[{server_id, workspace_path, gpu_id, cuda_visible_devices, gpu_cuda_visible_devices}]`；跨 endpoint 的兼容结果不生成有歧义的顶层 server/workspace/CUDA 字段。`workspace_path` 是所选服务器上的远端路径，不是本地 worktree 中的路径；Agent 必须通过当前已授权的远端 endpoint 进入它，不能在本地直接 `cd`。顶层与逐行兼容字段 `cuda_visible_devices` 是整个单 endpoint 资源集合，适合一个多卡进程；每行新增的 `gpu_cuda_visible_devices` 只含该卡 UUID，适合每卡一个进程。随后使用对应 selector 运行项目允许的最小 CUDA gate，确认它在该环境可用，再启动 workload。
