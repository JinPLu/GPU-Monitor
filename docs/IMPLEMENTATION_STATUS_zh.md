...
- 项目工作负载配置是可选的持久化资源合同：管理员一次定义项目、用途、GPU 数量、可选服务器子集和最大租约窗口；明确 `profile_id` 的 GUI/MCP 认领只传配置与任务，服务在立即或排队后分配时自动激活。没有 profile 时，一次性认领只需项目、任务和 GPU 数量，任务名自动记录为用途。申请不再要求预计占用时间，任务完成后释放租约；最大租约窗口是调度与未来预约共同遵守的硬边界。Agent 不得根据任务、目录或空闲容量自行挑选 profile，也不得推断项目或 GPU 数量。
- 已补齐跨项目调度的端点范围闭环：当 Broker 明确将请求阻挡为 `project_endpoint_scope` 时，管理员可通过 `gpu_grant_server_project` 为指定项目追加指定已登记服务器的访问权；不会覆盖其他项目，且会立即复核现有排队请求。该授权仍要求用户明确给出项目和服务器。
