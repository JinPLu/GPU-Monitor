...
2. `gpu_apply(server_id?, gpu_count=1, task?)` allocates GPUs. Never read a UI title or give a GPU ID. `ssh` is the connection; `workspace.path` is remote cwd (`kind=working_directory`, `use_as_cwd=true`); `code_location=not_provided` means no repository path. GPU UUIDs are identities, not selectors. Connect directly and `cd` to the workspace. Set `CUDA_DEVICE_ORDER=PCI_BUS_ID`; for one endpoint, `cuda_visible_devices` is the lease-wide ordinal set and each `gpus[]` row has a per-GPU ordinal in `gpu_cuda_visible_devices`. CUDA gate failure requires release and avoiding that server.
